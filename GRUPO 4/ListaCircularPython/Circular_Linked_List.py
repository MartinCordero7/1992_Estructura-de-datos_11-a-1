...
class CircularLinkedList:
    ...
    def insertar(self, nombre1, nombre2, apellido, correo, cedula, fecha):
        ...
        if not self.head:
            ...
            self.head.siguiente = self.head.anterior = self.head
        else:
    ...
    def eliminar(self, cedula):
        ...
        while True:
            if current.cedula == cedula:
                if previous is None:  # Eliminando el primer nodo
                    if current.siguiente == self.head:  # Solo un nodo
                        self.head = None
                    else:
                        last = self.head
                        while last.siguiente != self.head:
                            last = last.siguiente
                        self.head = current.siguiente
                        last.siguiente = self.head
                else:  # Eliminando un nodo intermedio o final
    ...
    def ordenar(self, criterio):
        if not self.head or self.head == self.tail:  # Solo un nodo o lista vacía
        ...
        # Convertir lista circular en una lista temporal
        nodes = []
        current = self.head
        while current:
            nodes.append(current)
            current = current.siguiente
            if current == self.head:  # Volvimos al inicio
                break

        # Ordenar la lista de nodos
        nodes.sort(key=lambda n: key_func(n))

        # Reconstruir la lista circular con los nodos ordenados
        self.head = nodes[0]
        self.tail = nodes[-1]
        for i in range(len(nodes)):
            nodes[i].siguiente = nodes[i + 1] if i + 1 < len(nodes) else self.head
            nodes[i].anterior = nodes[i - 1] if i > 0 else self.tail
    ...
    def buscar(self, cedula):
        ...
        while True:
            if current.cedula == cedula:
                print(f"Persona encontrada: {current.nombre1} {current.nombre2} {current.apellido}, Correo: {current.correo}, Cédula: {current.cedula}, Fecha de Nacimiento: {current.fecha}")
                return
            current = current.siguiente
            if current == self.head:  # Hemos recorrido toda la lista y regresado al inicio
                break
        ...
        while True:
            if current.cedula == cedula:
                if current == self.head:  # Si es el primer nodo
                    if self.head == self.tail:  # Solo hay un nodo
                        self.head = self.tail = None
                    else:
                        self.head = current.siguiente
                        self.head.anterior = self.tail
                        self.tail.siguiente = self.head
                elif current == self.tail:  # Si es el último nodo
                ...
                return
            current = current.siguiente
            if current == self.head:  # Hemos recorrido toda la lista
                break
