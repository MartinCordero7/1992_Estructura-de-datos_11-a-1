...
bool Validaciones::validarIsni(const string& isni) {
    ...
    // Verificar que todos los caracteres no sean iguales
    if (std::all_of(isniSinEspacios.begin(), isniSinEspacios.end(), [isniSinEspacios](char c) { return c == isniSinEspacios[0]; })) {
    ...
    int controlDigit = (12 - (sum % 11)) % 11;
    if (controlDigit == 10) {
        return lastChar == 'X';
    } else {
        return lastChar == (controlDigit + '0');
    }
}
...
bool Validaciones::validarOpcion(const string& opcion) {
    regex formatoOpcion(R"(\d+)");
    if (!regex_match(opcion, formatoOpcion)) {
        cout << "Error: La opción debe ser un número.\n";
        return false;
    }
    return true;
}
...
bool Validaciones::validarPalabra(const string& palabra) {
    regex formatoPalabra(R"([a-zA-ZñÑáéíóúÁÉÍÓÚ]+)");
    if (palabra.empty() || palabra.find_first_not_of(' ') == string::npos) {
        cout << "Error: La palabra no puede estar vacía o contener solo espacios.\n";
        return false;
    } else if (!regex_match(palabra, formatoPalabra)) {
        cout << "Error: La palabra solo debe contener letras y caracteres válidos en español.\n";
        return false;
    }
    return true;
}
